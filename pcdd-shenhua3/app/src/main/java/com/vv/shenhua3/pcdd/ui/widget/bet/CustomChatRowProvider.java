package com.vv.shenhua3.pcdd.ui.widget.bet;
...
public class CustomChatRowProvider implements EaseCustomChatRowProvider {
    ...
    private static final int ROW_COUNT_DOWN = 7;

    private int rowTypeCount = 7;   //消息类型，必须匹配
    ...
    public EaseChatRow getCustomChatRow(EMMessage message, int position, BaseAdapter adapter) {
        switch(getCustomChatRowType(message)) {
            ...
            case ROW_COUNT_DOWN:
                return new EaseChatRowCountDown(context, message, position, adapter);
        }
        return null;
    }
}

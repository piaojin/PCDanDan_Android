package com.vv.shenhua3.pcdd.ui.widget.refresh;
...
public class PullLoadMoreRecyclerView extends LinearLayout {
    ...
    private void initView(Context context) {
        mContext = context;
        View view = LayoutInflater.from(context).inflate(com.vv.shenhua3.pcdd.R.layout.pull_loadmore_layout, null);

        mSwipeRefreshLayout = (SwipeRefreshLayout) view.findViewById(com.vv.shenhua3.pcdd.R.id.swipeRefreshLayout);
        mSwipeRefreshLayout.setColorSchemeResources(android.R.color.holo_green_dark, android.R.color.holo_blue_dark, android.R.color.holo_orange_dark);
        mSwipeRefreshLayout.setOnRefreshListener(new SwipeRefreshLayoutOnRefresh(this));

        mRecyclerView = (RecyclerView) view.findViewById(com.vv.shenhua3.pcdd.R.id.recycler_view);
    ...
    private float downY;
    private float pullUpDistance = 80;  //上拉刷新触发距离

    /**
     * 设置recycleview在一屏能显示的情况下的上拉刷新
     */
    public void setLessDataLoadMore() {
        new Handler().postDelayed(new Runnable() {
            @Override
            public void run() {
                int lastItemPosi = ((LinearLayoutManager) mRecyclerView.getLayoutManager()).findLastCompletelyVisibleItemPosition();
                if(lastItemPosi == mRecyclerView.getAdapter().getItemCount()-1) {
